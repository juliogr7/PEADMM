...
def init_parser():
    ...
    parser.add_argument('--max-epochs', default=201, type=int,
    ...
    parser.add_argument('--cr', default=0.1, type=float,
                        help='Compress ratio for the Single-Pixel Camera')
...
def main():
    ...
    config = (f"SPI_Encoder_elu_{args.elu_encoder}_GAN_elu_{args.elu_gan}_{date}_cr_{args.cr}_epochs_{args.max_epochs}")
    ...
    spi_train_dataset = torch.load(
        os.path.join(args.data_dir, f"spi_train_loader_cr_{str(args.cr).replace('.', '')}.pt"),
        weights_only=False)
    train_loader = torch.utils.data.DataLoader(spi_train_dataset, batch_size=args.batch_size,
                                                shuffle=True,
                                                num_workers=args.num_workers, pin_memory=True,
                                                persistent_workers=True)
    print("SPI train dataset loaded successfully.")

    spi_valid_dataset = torch.load(
        os.path.join(args.data_dir, f"spi_valid_loader_cr_{str(args.cr).replace('.', '')}.pt"),
        weights_only=False)
    valid_loader = torch.utils.data.DataLoader(spi_valid_dataset, batch_size=args.batch_size,
                                                shuffle=False,
                                                num_workers=args.num_workers, pin_memory=True,
                                                persistent_workers=True)
    print("SPI valid dataset loaded successfully.")

    spi_test_dataset = torch.load(
        os.path.join(args.data_dir, f"spi_test_loader_cr_{str(args.cr).replace('.', '')}.pt"),
        weights_only=False)
    test_loader = torch.utils.data.DataLoader(spi_test_dataset, batch_size=args.batch_size,
                                                shuffle=False,
                                                num_workers=args.num_workers, pin_memory=True,
                                                persistent_workers=True)
    print("SPI test dataset loaded successfully.")
    ...
    wdb_logger = WandbLogger(project='project_name', name=args.save_name, save_code=False, log_model=False)
